...
bool JVM::Init(int argc, const char* argv[])
{
...
#else
    this->module = dlopen("/Library/Internet Plug-Ins/JavaAppletPlugin.plugin/Contents/Home/lib/server/libjvm.dylib", RTLD_LAZY);
#endif // defined
    ...
    if (argc > 0)
    {
        ...
        if (JNI_GetCreatedJavaVMs(vms, max_vms, &num_vms) == JNI_OK && num_vms > 0)
        {
